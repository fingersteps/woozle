...
namespace Woozle.Persistence.Ef.Repository
{
    public partial class RoleRepository  : AbstractRepository<Role>
    {
    	...
    	catch (Exception e)
    	{
    	 ...
    	 public override void Delete(Role entity, Session session) 
    	 { 
    		try
    		{
    			var stopwatch = new Stopwatch();
    			...
    			//Navigation Property 'MandatorRoles'
    			stopwatch.Start();
    			Context.LoadCollection<Role>(attachedObj.Id, "MandatorRoles");
    			...
    			Context.Commit();
    			stopwatch.Stop();
    			this.Logger.Info(string.Format("Commit '{0}' Delete, took {1} ms", "Role", stopwatch.ElapsedMilliseconds));
    		}
    	catch (Exception e)
    	{
    		this.Logger.Error(e.Message); 
