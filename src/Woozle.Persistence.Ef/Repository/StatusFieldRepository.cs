...
{
    public partial class StatusFieldRepository  : AbstractRepository<StatusField>
    {
    
    	public StatusFieldRepository(IEfUnitOfWork Context) : base(Context)
    	{
    	}
    
    
    	 public override StatusField Synchronize(StatusField entity, Session session) 
    	 { 
    		try
    		{
    	...
    	catch (Exception e)
    	{
    	 ...
    	 public override void Delete(StatusField entity, Session session) 
    	 { 
    		try
    		{
    			var stopwatch = new Stopwatch();
    			...
    			//Navigation Property 'Status'
    			stopwatch.Start();
    			Context.LoadCollection<StatusField>(attachedObj.Id, "Status");
    			...
    			Context.Commit();
    			stopwatch.Stop();
    			this.Logger.Info(string.Format("Commit '{0}' Delete, took {1} ms", "StatusField", stopwatch.ElapsedMilliseconds));
    		}
    	catch (Exception e)
    	{
    		this.Logger.Error(e.Message); 
